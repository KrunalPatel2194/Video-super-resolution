...
# its inputs extend it to a spatio-temporal D
...
def modelnet(r_inputs, r_targets, FLAGS, GAN_Flag=True):
    ...
    if(GAN_Flag):# build the discriminator
        ...
        with tf.device('/gpu:0'), tf.name_scope('input_Tdiscriminator'):
            ...
            T_vel = tf.stop_gradient( T_vel ) # won't passing gradient to fnet from discriminator
            
        if(FLAGS.crop_dt < 1.0): # crop out unstable part for temporal discriminator 
...
def FRVSR(r_inputs, r_targets, FLAGS):
    return modelnet(r_inputs, r_targets, FLAGS, False)