...
def loadHR(FLAGS, tar_size):
    ...
    with tf.device('/cpu:0'):
        ...
        image_list_HR_r = [[] for _ in range( FLAGS.RNN_N )] # Creating 10 empty lists in a list
        ...
        print("num_image_list_HR_t_cur :",num_image_list_HR_t_cur)
        ...
        with tf.variable_scope('load_frame'):
            ...
            output = tf.train.slice_input_producer(image_list_HR_r, shuffle=False,\
                capacity=int(FLAGS.name_video_queue_capacity) )
            output_names = output #tensors list of queue 1024
